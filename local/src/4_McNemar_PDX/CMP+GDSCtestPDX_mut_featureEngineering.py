...
Mut = PDX_features_in
Y = pd.read_csv(f, sep="\t", index_col=1, header=0).set_index('ircc_id')
# encode target variable (Cetuximab response 3w)
...
PDX_all_df = pd.merge(Mut, Y[target_col], right_index=True, left_index=True, how="right")
...
Y = drug_response_data[['SANGER_MODEL_ID', 
    'is_test', target_col]].set_index('SANGER_MODEL_ID')
feature_col = Mut.columns
