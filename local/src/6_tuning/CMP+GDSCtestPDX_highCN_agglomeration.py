...
# # Introduction
# 
# Compute CNV stats for intogen genes that map to CMP model segmented CN data (from gistic)
...
Y = drug_response_data[['SANGER_MODEL_ID', CMPGDSC_target_col]].\
...
PDX_X_test = pd.DataFrame(MinMaxScaler().fit_transform(PDX_X_test.values),
                            columns=PDX_X_test.columns,
                            index=PDX_X_test.index)
