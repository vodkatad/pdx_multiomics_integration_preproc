...
Meth = Meth[Meth.columns.drop(list(Meth.filter(regex='Cetuximab|ircc_id')))]
...
Expr = Expr[Expr.columns.drop(list(Expr.filter(regex='Cetuximab|ircc_id')))]
...
rawMeth = rawMeth[rawMeth.columns.drop(
    list(rawMeth.filter(regex='Cetuximab|ircc_id')))]
...
rawExpr = rawExpr[rawExpr.columns.drop(
    list(rawExpr.filter(regex='Cetuximab|ircc_id')))]
...
CNV = CNV[CNV.columns.drop(list(CNV.filter(regex='Cetuximab|ircc_id')))]
...
Mut = Mut[Mut.columns.drop(list(Mut.filter(regex='Cetuximab|ircc_id')))]
...
Clin = Clin[Clin.columns.drop(list(Clin.filter(regex='Cetuximab|ircc_id')))]
...
# L1-penalised (Lasso) multivariate feature selection
L1LR = LogisticRegression(
    penalty='l1',
    solver='saga')
# variace for feature selection
def calc_variance(X,y):
    X_df = pd.DataFrame(X)
    # return a single array with scores
    return X_df.var().values
...
pipe_steps = [
    ...
    ("bestVarFilter", SelectKBest(chi2, k=2000)),
...
pipe_steps = [
   ...
   #("bestVarFilter", SelectKBest(calc_variance, k=3000)),
    ("L1Selector", L1Selector),
    ('elasticNetClassifier', elasticNetClassifier),
]
...
hyperparameter_grid = {
    ...
    # 'pipeline-1__': [],
    # Expr params
    'pipeline-2__chi2filterFscore__k': [20],
    ...
    # rawExpr pipeline
    #'pipeline-3__L1Selector__estimator__C': [50],
    #'pipeline-3__elasticNetClassifier__l1_ratio': np.linspace(.01, .9, 3, endpoint=True),
    # rawMeth pipeline
    #'pipeline-4__L1Selector__estimator__C': [50],
    #'pipeline-4__elasticNetClassifier__l1_ratio': [.9],
    # Mut params
    'pipeline-5__chi2filterFscore__k': [50],
    ...
    'meta_classifier__l1_ratio':  np.linspace(.01, .9, 3, endpoint=True)
}
