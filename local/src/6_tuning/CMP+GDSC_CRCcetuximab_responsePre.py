...
CMP_CRC_lines = CMP_meta_data[CMP_meta_data.cancer_type ==
                              "Colorectal Carcinoma"].model_name.unique()
CMP_CRC_sangerID = CMP_meta_data[CMP_meta_data.cancer_type ==
                                 "Colorectal Carcinoma"].model_id.unique()

# add GDSC drug response data
GDSC_response_data = pd.concat([pd.read_csv(snakemake.input.GDSC1),
                                pd.read_csv(snakemake.input.GDSC2)]).drop_duplicates()
GDSC_response_data = GDSC_response_data.rename(
    columns={"DRUG_NAME": "DRUG_NAME_response"})

# obtain IC50
GDSC_response_data["IC50"] = GDSC_response_data["LN_IC50"].astype(
    float). apply(lambda x: math.exp(x))
# check if IC50<max_c
GDSC_response_data["IC50<MAX_CONC"] = GDSC_response_data.IC50 < GDSC_response_data.MAX_CONC
# compute the IC50/MAX_C ratio
GDSC_response_data["IC50_ratio"] = GDSC_response_data.IC50 / \
    GDSC_response_data.MAX_CONC
GDSC_response_data.COSMIC_ID = GDSC_response_data.COSMIC_ID.astype(
    int).astype(str)
# filter for CMP CRC lines screened with Cetuximab
GDSC_cetux_lines = GDSC_response_data[GDSC_response_data.DRUG_NAME_response ==
                                      'Cetuximab'].CELL_LINE_NAME.unique()
GDSC_cetux_sangerID = GDSC_response_data[GDSC_response_data.DRUG_NAME_response ==
                                         'Cetuximab'].SANGER_MODEL_ID.unique()
...
GDSC_CRCcetuxi_response_data['IC50_byMedian_cat'] = (
    GDSC_CRCcetuxi_response_data.IC50 < IC50_median).replace({True: 1, False: 0})
GDSC_CRCcetuxi_response_data['IC50_byMedian_cat'].value_counts()
# calc IC50 Zscore
GDSC_CRCcetuxi_response_data['IC50_Zscore'] = zscore(
    GDSC_CRCcetuxi_response_data.IC50.values)
